...
class ChaincoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some chaincoind methods
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = chaincoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = chaincoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = chaincoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        getnetinfo = self.rpc_command('getnetworkinfo')
        return (getnetinfo['protocolversion'] >= config.min_chaincoind_proto_version_with_sentinel_ping)
