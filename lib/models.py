...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, chaincoind):
        golist = chaincoind.rpc_command('gobject', 'list')
    ...
    def import_gobject_from_chaincoind(self, chaincoind, rec):
        ...
        # exclude any invalid model data from chaincoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from chaincoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from chaincoind - Chaincoind is the master
        try:
            ...
            printdbg("Got invalid object from chaincoind! %s" % e)
            govobj.vote_delete(chaincoind)
            return (govobj, None)
    ...
    def vote_delete(self, chaincoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(chaincoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, chaincoind, signal, outcome):
        ...
        output = chaincoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = chaincoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(chaincoind, signal)

    def sync_network_vote(self, chaincoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = chaincoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = CHAINCOIND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self, chaincoind):
        ...
        try:
            ...
            # Chaincoin Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (chaincoinlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = CHAINCOIND_GOVOBJ_TYPES['superblock']
    ...
    def hash(self):
        import chaincoinlib
        return chaincoinlib.hashit(self.serialise())
